...
**Lock Screen:** By default, Computer Use is blocked while macOS is locked (fail-closed). Absent lock-state evidence is corroborated by the on-console signal — when the session is on-console, absent evidence is treated as unlocked; when off-console, or when evidence is nil or unparseable, it is treated as locked. A screensaver without a password requirement is presence-blind and reads as unlocked (not a regression this fix owns). For fully attended, screenshot-driven automation, unlock the Mac or use a dedicated logged-in desktop session.
