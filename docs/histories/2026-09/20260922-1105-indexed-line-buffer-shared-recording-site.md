...
- **[New type]**: `IndexedLineBuffer` owns `lines` and `offsets` as `private(set)` state and
  ...
  above it. The pair `lines.append(...)` / `lineOffsets[index] = lines.count - 1` no longer exists
  anywhere, so there is no gap between the two statements for a later edit to insert a row into.
...
- **[New test]**: `testSpanRowsDoNotShiftTheOffsetsOfTheIndexedRowsAroundThem` drives the buffer
  directly with a span row landing between two indexed rows. Because both renderers now share this
  implementation, this is the first coverage the live-AX recording site has ever had.
...
makes the live path testable at all, since the tested buffer is now literally the code it runs.
