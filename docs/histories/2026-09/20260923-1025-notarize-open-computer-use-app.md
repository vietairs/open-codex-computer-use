...
- **Timestamp tied to that decision**: `codesign_app_bundle()` passes `--timestamp` when `will_notarize=1` and `--timestamp=none` for non-ad-hoc debug builds. codesign requests a secure timestamp by default for Developer ID signatures, so the explicit `none` is what keeps the debug loop offline; release builds that are not notarized keep that default timestamp, as before. `notarytool submit` waits at most 30 minutes.
