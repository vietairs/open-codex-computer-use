...
- [x] 14. FUS-via-SSH gate — same-user branch confirmed (416 live samples, all inferred-unlocked); live off-console attempt with user BLOCKED (no SSH, Fast User Switching disabled on this Mac, only Log Out available — logout is destructive, not equivalent to the test); automated SACSwitchToLoginWindow call also returns EINVAL from background. User explicitly accepted this as residual risk rather than force a system-setting change/logout → PR #5 body + checklist updated (item now closed as accepted risk), comment posted (5018428722); reports/fus-via-ssh-gate-verification-260720-*.md. CLOSED — no live off-console verification obtained, documented limitation.
- [x] 15. merge + close — PR #5 merged by user request (squash, mergeCommit efd7214, main now at efd7214), auto-closed by GitHub
- [x] 16. build — `./scripts/build-open-computer-use-app.sh release --arch native` → `dist/Open Computer Use.app`, signed with Developer ID Application: Viet Nguyen (3HB354R355)
- [x] 17. deploy — copied signed bundle to `/Applications/Open Computer Use.app`, launched and confirmed running (PID 59679); added to macOS Login Items for easy/auto open (user chose this over Dock shortcut — app is LSUIElement, no Dock icon while running)
- [ ] 18. deferred: worktree teardown — NOT run this session (this job's cwd is the worktree itself; removing it mid-run would kill the running session). origin/main is already at efd7214 (fetched, confirmed) — base sync only needs `git pull --ff-only` in the primary checkout, not this worktree. User/primary session should run `/hvn-worktree` teardown (or `git worktree remove`) for `fix/mac-session-guard-lock-detection` when convenient.
